...
- **Tests:** Add unit tests for services and repositories, and e2e tests covering the full request lifecycle for each endpoint.
- **Observability:** Add structured logging (e.g. with a logger like Pino), request tracing, and health check endpoints. Integrate with Datadog for centralised log aggregation, APM traces, and dashboards. Set up Datadog monitors and alerts on key metrics (error rates, response times, DB connection health) so issues are caught before users report them.
